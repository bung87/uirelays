...
template linkModPressed(mods: set[Modifier]): bool =
  when defined(macosx):
    GuiPressed in mods or CtrlPressed in mods
  else:
    CtrlPressed in mods
...
proc draw*(s: var SynEdit; e: Event; area: Rect; focused: bool): EditAction =
  ...
  if e.kind == MouseDownEvent and linkModPressed(e.mods) and
